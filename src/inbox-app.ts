...
import {PromiseWrapper, ObservableWrapper, Promise}  from 'angular2/src/facade/async'
...
@View({templateUrl: 'src/inbox-detail.html', directives: [NgFor, RouterLink]})
class InboxDetailCmp {
	...
	constructor(db: DbService, params: RouteParams) {
		...
		PromiseWrapper.then( db.email(id), (data) => { 
...
@View({templateUrl: 'src/inbox.html', directives: [NgFor, RouterLink]})
class InboxCmp {
	items: InboxRecord[] = [];
	ready: boolean = false;
	...
	constructor(public router: Router, db: DbService, params: RouteParams) {
		...
		PromiseWrapper.then(db.emails(), emails => {
			...
			if (sortEmailsByDate) {
				ListWrapper.sort(this.items, (a, b) => {
					let dateA = DateWrapper.toMillis(DateWrapper.fromISOString(a.date));
					let dateB = DateWrapper.toMillis(DateWrapper.fromISOString(b.date));
...
@View({templateUrl: 'src/drafts.html', directives: [NgFor, RouterLink]})
class DraftsCmp {
	items: InboxRecord[] = [];
	ready: boolean = false;
...
@View({templateUrl: 'src/inbox-app.html', directives: [RouterOutlet, RouterLink]})
@RouteConfig([
...
export class InboxApp {
	...
	draftsPageActive() { return this.location.path() == '/draft'; }
}